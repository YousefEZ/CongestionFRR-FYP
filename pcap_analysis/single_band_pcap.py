...
def record_flow_completion_time(source_directory, result_directory, mode):
    ...
    results = {
        i: {case: generate_variable_dict(variables) for case in cases} for i in (1,)
    }
                ...
                for case in cases:
                    for queue_size in queue_sizes:
                        if (
                            os.path.isdir(
                                os.path.join(
                                    source_directory + variable + "/" + queue_size
                                )
                            )
                            and queue_size != "99"
                        ):
                            if case == "udp":
                                results[senders][case][variable][queue_size] = (
                                    get_avg_fc_time(
                                        f"{source_directory}{variable}/{queue_size}/frr",
                                        senders,
                                    )
                                )
                            else:
                                results[senders][case][variable][queue_size] = (
                                    get_avg_fc_time(
                                        f"{source_directory}{variable}/{queue_size}/frr-{case}",
                                        senders,
                                    )
                                )
                    results[senders][case][variable]["base"] = get_avg_fc_time(
                        f"{source_directory}{variable}/50/baseline-{case}", senders
                    )
                ...
                for case, variables in case_results.items():
                    for variable, queue_sizes in variables.items():
                        for queue_size, result in queue_sizes.items():
                            f.write(
                                f"senders={senders} {case} {variable} {queue_size} {result}\n"
                            )
...
def plot_flow_completion_time(results, mode, cases):
    sorted_results = sorted(
        results,
        key=lambda x: int(x[3].split("Mbps")[0])
        if "Mbps" in x[3]
        else int(x[3].split("ms")[0]),
    )
    figure, axes = plt.subplots(figsize=(10, 6))

    marker_styles = {
        "baseline": {"marker": "o", "colour": "black"},
        "50": {"marker": "s", "colour": "blue"},
    }
        ...
        if result is not None and result[2] is not None:
            if cases[0] == result[0]:
                ...
                if "baseline" not in legend_handles:
                    (legend_handles["baseline"],) = axes.plot(
                        [],
                        [],
                        marker=marker_style["marker"],
                        color=marker_style["colour"],
            ...
            if cases[1] == result[0]:
                ...
                if result[1] not in legend_handles:
                    (legend_handles[result[1]],) = axes.plot(
                        [],
                        [],
                        marker=marker_style["marker"],
                        color=marker_style["colour"],
        ...
        for i in range(1, len(points)):
            if points[i][0] == cases[0]:
                axes.plot(
                    ...
                    color=marker_styles["baseline"]["colour"],
                    linestyle="-",
                )
            else:
                axes.plot(
                    [points[i - 1][3], points[i][3]],
                    [points[i - 1][2], points[i][2]],
                    color=marker_styles[points[i][1]]["colour"],
                    linestyle="-",
                )
...
if __name__ == "__main__":
    ...
    bandwidth_primary_results = record_flow_completion_time(
        "../traces/bandwidth_primary/", "../pcap_analysis/results", "bandwidth_primary"
    )
    ...
    plot_flow_completion_time(
        bandwidth_primary_results, "bandwidth_primary", ["baseline_udp", "frr"]
    )
    # with udp
    ...
    # delay_primary_results = record_flow_completion_time(
    #    "../traces/delay_primary/", "../pcap_analysis/results", "delay_primary"
    # )
    ...
    # delay_alternate_results = record_flow_completion_time(
    #    "../traces/delay_alternate/", "../pcap_analysis/results", "delay_alternate"
    # )
