...
SequenceNumber32 TcpTxBuffer::HeadSequence() const
{
...
uint32_t TcpTxBuffer::Size() const
{
    return m_size;
}

uint32_t TcpTxBuffer::MaxBufferSize() const
{
    return m_maxBuffer;
}
...
bool TcpTxBuffer::IsSackEnabled() const
{
    return m_sackEnabled;
}

void TcpTxBuffer::SetSackEnabled(bool enabled)
{
    m_sackEnabled = enabled;
}

uint32_t TcpTxBuffer::Available() const
{
...
uint32_t TcpTxBuffer::GetRetransmitsCount() const
{
    return m_retrans;
}

uint32_t TcpTxBuffer::GetLost() const
{
    return m_lostOut;
}

uint32_t TcpTxBuffer::GetSacked() const
{
    return m_sackedOut;
}
    ...
    for (; it != m_sentList.end(); ++it) {
        if ((*it)->m_startSeq == seq) {
            ...
            if (next != m_sentList.end()) {
                // Next is not sacked and have the same value for m_lost ...
                // there is the possibility to merge
                if ((!(*next)->m_sacked) &&
                    ((*it)->m_lost == (*next)->m_lost)) {
                    s = std::min(s, (*it)->m_packet->GetSize() +
                                        (*next)->m_packet->GetSize());
                } else {
                    // Next is sacked... better to retransmit only the first
                    // segment
                    s = std::min(s, (*it)->m_packet->GetSize());
                }
            } else {
                                          ...
                                          bool* listEdited) const
{
     ...
     *
     * In (1), things are pretty easy, it's just a matter of walking the list
     * and defragment packets, if needed (e.g. seq is the beginning of the first
    ...
    while (it != list.end()) {
        ...
        if (seq < beginOfCurrentPacket + currentPacket->GetSize()) {
            ...
            if (seq == beginOfCurrentPacket) {
                ...
                // seq is inside the current packet but seq is not the
                // beginning, it's somewhere in the middle. Just fragment the
        ...
        if (seq + numBytes <= beginOfCurrentPacket + currentPacket->GetSize()) {
            ...
            if (numBytes == currentPacket->GetSize()) {
                // the end boundary is exactly the end of the current packet.
                // Hurray!
            ...
            // The end isn't inside current packet, but there is an exception
            // for the merge and recurse strategy...
            if (++it == list.end()) {
            ...
            // The current packet does not contain the requested end. Merge
            // current with the packet that follows, and recurse
                              ...
                              const Callback<void, TcpTxItem*>& beforeDelCb)
{
    ...
    while (m_size > 0 && offset > 0) {
        ...
        if (offset >= pktSize) { // This packet is behind the seqnum. Remove
                                 // this packet from the buffer
            m_size -= pktSize;
            m_sentSize -= pktSize;
            offset -= pktSize;
                             ...
                             const Callback<void, TcpTxItem*>& sackedCb)
{
    ...
    for (auto option_it = list.begin(); option_it != list.end(); ++option_it) {
        ...
        if (m_firstByteSeq + m_sentSize < (*option_it).first) {
            NS_LOG_INFO("Not updating scoreboard, the option block is outside "
                        "the sent list");
            return bytesSacked;
        }
        ...
        while (item_it != m_sentList.end()) {
            ...
            // it is precisely mapped over the option. It means that if the
            // receiver is reporting as sacked single range bytes that are not
            // mapped 1:1 in what we have, the option is discarded. There's room
            ...
            if (beginOfCurrentPacket >= (*option_it).first &&
                ...
                if ((*item_it)->m_sacked) {
                    ...
                    if (m_highestSack.first == m_sentList.end() ||
                        ...
                        m_sackSeen = true;
                        m_highestSack =
                            std::make_pair(item_it, beginOfCurrentPacket);
                    }

                    if ((*item_it)->m_retrans) {
                        m_retrans -= (*item_it)->m_packet->GetSize();
                        NS_LOG_INFO("Sacked retransmitted data seq="
                                    << (*item_it)->m_startSeq);
                    }
                    ...
                    NS_LOG_INFO("Received block "
                                ...
                                << ", checking sentList for block "
                                << *(*item_it)
                                << ", found in the sackboard, sacking, current "
                                   "highSack: "
                                << m_highestSack.second);
    ...
    // Assert for duplicated SACK or impossibility to map the option into the
    // sent blocks NS_ASSERT (list.size () == 0 || modified);
    ConsistencyCheck();
    return bytesSacked;
}
...
bool TcpTxBuffer::IsLost(const SequenceNumber32& seq) const
{
    ...
    for (auto it = m_sentList.begin(); it != m_sentList.end(); ++it) {
        ...
        if ((*it)->m_startSeq <= seq &&
            seq < (*it)->m_startSeq + (*it)->m_packet->GetSize()) {
            if ((*it)->m_lost) {
                          ...
                          bool isRecovery) const
{
    ...
    for (auto it = m_sentList.begin(); it != m_sentList.end(); ++it) {
        ...
        if (!item->m_retrans && !item->m_sacked &&
            ((m_sackSeen && item->m_startSeq < m_highestSack.second) ||
             !m_sackSeen)) {
            if (item->m_lost) {
    ...
    if (SizeFromSequence(m_firstByteSeq + m_sentSize) > 0) {
        if (m_sentSize < m_rWndCallback()) {
            ...
            *seqHigh =
                *seq + std::min<uint32_t>(m_segmentSize,
...
uint32_t TcpTxBuffer::BytesInFlight() const
{
    ...
    //                              " retrans: " << retrans);
    NS_LOG_INFO("Calculated BytesInFlight: " << in_flight);
    return in_flight;
}
...
uint32_t TcpTxBuffer::BytesInFlightRFC() const
{
    ...
    // octet 'S1' in the sequence space between HighACK and HighData that has
    // not been SACKed:
    for (auto it = m_sentList.begin(); it != m_sentList.end(); ++it) {
        item = *it;
    ...
    NS_ASSERT_MSG(sackedOut == m_sackedOut,
                  "Sacked counted: " << sackedOut << " " << m_sackedOut
                                     << *this);
    ...
    for (it = segment; it != m_sentList.end(); ++it) {
        ...
        if (item->m_sacked) {
            NS_LOG_INFO("Segment " << *item
                                   << " found to be SACKed while checking for "
                                   << seq);
            ++count;
    ...
    for (auto it = m_sentList.begin(); it != m_sentList.end(); ++it) {
        if (resetSack) {
            ...
            if ((*it)->m_lost) {
                ...
                // Packet is not marked lost, nor is sacked. Then it becomes
                // lost.
                (*it)->m_lost = true;
                m_lostOut += (*it)->m_packet->GetSize();
            }
...
void TcpTxBuffer::MarkHeadAsLost()
{
    if (!m_sentList.empty()) {
        ...
        // A sacked head means that we should advance SND.UNA.. so it's an
        // error.
