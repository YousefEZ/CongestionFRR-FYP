#ifndef EMPTY_REROUTE_HEAD_POLICY_H
#define EMPTY_REROUTE_HEAD_POLICY_H
...
struct EmptyRerouteHeadPolicy;

struct EmptyRerouteHeadPolicy : public LFAPolicy {
    EmptyRerouteHeadPolicy() = default;
    virtual ~EmptyRerouteHeadPolicy() = default;
...
bool EmptyRerouteHeadPolicy::isAlternateQueueCongested()
{
    Ptr<Queue<Packet>> alternateQueue = m_alternate->GetQueue();
    return alternateQueue->GetNPackets() == 0;
}

bool EmptyRerouteHeadPolicy::handlePacket(Ptr<Packet> newPacket,
                                         const Address& dest,
                                         uint16_t protocolNumber,
