#ifndef SAFE_REROUTE_HEAD_POLICY_H
#define SAFE_REROUTE_HEAD_POLICY_H
...
struct SafeRerouteHeadPolicy;

struct SafeRerouteHeadPolicy : public LFAPolicy {
    SafeRerouteHeadPolicy() = default;
    virtual ~SafeRerouteHeadPolicy() = default;
...
bool SafeRerouteHeadPolicy::isAlternateQueueCongested()
{
    Ptr<Queue<Packet>> alternateQueue = m_alternate->GetQueue();
    return alternateQueue->GetNPackets() == alternateQueue->GetMaxSize().GetValue();
}

bool SafeRerouteHeadPolicy::handlePacket(Ptr<Packet> newPacket,
                                         const Address& dest,
                                         uint16_t protocolNumber,
